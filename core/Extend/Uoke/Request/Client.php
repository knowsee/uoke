...
class Client {
    ...
    public function getCookieName($name) {
        $prefix = \app::$coreConfig['cookies']['prefix'];
        return $prefix . $name;
    }

    public function getCookies($list) {
        ...
        foreach ($list as $name) {
            ...
            if (isset($this->_cookieParams[$cookiesName])) {
                $return[$name] = $this->_cookieParams[$cookiesName];
            } else {
                $return[$name] = null;
            }
        }
    ...
    public function setCookies($name, $value = null, $expire = null) {
        if (empty($expire)) {
            $expire = \app::$coreConfig['cookies']['lifetime'];
        }
        $expire = $expire + UNIXTIME;
        if (!is_array($name)) {
            $cookiesList = array(
                $name => $value
            );
    ...
    public function deleteCookies($name) {
        ...
        return $this->setCookie($name, null, -3600);
    }
