...
// 签名证书路径
    const SDK_SIGN_CERT_PATH = SYSTEM_PATH.'Data/UnionPay/acp_test_sign.pfx';

// 签名证书密码
    const SDK_SIGN_CERT_PWD = '000000';

// 密码加密证书（这条一般用不到的请随便配）
    const SDK_ENCRYPT_CERT_PATH = SYSTEM_PATH.'Data/UnionPay/acp_test_enc.cer';

// 验签证书路径（请配到文件夹，不要配到具体文件）
    const SDK_VERIFY_CERT_DIR = SYSTEM_PATH.'Data/UnionPay/';
...
// 前台通知地址 (商户自行配置通知地址)
    const SDK_FRONT_NOTIFY_URL = 'http://localhost:8085/upacp_demo_df/demo/api_10_df/FrontReceive.php';

// 后台通知地址 (商户自行配置通知地址，需配置外网能访问的地址)
    const SDK_BACK_NOTIFY_URL = 'http://222.222.222.222/uoke/BackReceive.php';

//文件下载目录
    const SDK_FILE_DOWN_PATH = SYSTEM_PATH.'Data/UnionPay/Down/';

//日志 目录
    const SDK_LOG_FILE_PATH = SYSTEM_PATH.'Data/UnionPay/Log/';
